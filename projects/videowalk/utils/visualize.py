...
def pca_feats(ff, K=1, solver='auto', whiten=True, img_normalize=True):
    ...
    N, C, H, W = ff.shape
    pca = PCA(n_components=3 * K, svd_solver=solver, whiten=whiten)
...
def draw_matches(x1, x2, i1, i2):
    ...
    for check in [True]:
        ...
        scale = i1.shape[-2] / h
        grid = torch.stack([
            torch.arange(0, h)[None].repeat(h, 1),
            torch.arange(0, h)[:, None].repeat(1, h)
        ])
        ...
        kps = [
            cv2.KeyPoint(grid[0][i], grid[1][i], 1)
...
def nn_patches(vis, P, A_k, prefix='', N=10, K=20):
    ...
    vis.text(
        '', opts=dict(width=10000, height=1), win='%s_patch_header' % (prefix))

    for n, i in enumerate(np.random.permutation(P.shape[0])[:N]):
        p = P[i]
        vis.text(
            '',
            opts=dict(width=10000, height=1),
        ...
        for k in range(I.shape[0]):
            vis.images(
                P[I[k, i, :K]],
                nrow=min(I.shape[-1], 20),
                win='%s_patch_values_%s_%s' % (prefix, n, k))
            vis.bar(
                A[k, i][::-1][:K],
                opts=dict(height=150, width=500),
...
def vis_flow_plt(u, v, x1, x2):
    ...
    ax.quiver(my[skip], mx[skip], flows[..., 0][skip],
              flows[..., 1][skip] * -1, C)  #, scale=1, scale_units='dots')
...
def frame_pair(x, ff, mm, t1, t2, A, AA, xent_loss, viz):
    ...
    if len(x.shape) < 6:  # Single image input, no patches
        ...
        # # PCA VIZ
        spatialize = lambda xx: xx.view(*xx.shape[:-1], int(xx.shape[-1]**0.5),
                                        int(xx.shape[-1]**0.5))
        ...
        viz.images(
            pca_ff.transpose(0, -1, 1, 2),
            win='pcafeats',
            opts=dict(title=f'{t1} {t2}'))
        ...
        pca1 = torchvision.utils.make_grid(
            torch.Tensor(pca_ff[:N]), nrow=int(N**0.5), padding=1, pad_value=1)
        pca2 = torchvision.utils.make_grid(
            torch.Tensor(pca_ff[N:]), nrow=int(N**0.5), padding=1, pad_value=1)
    ...
    img_grid = [
        cv2.resize(aa, (50, 50), interpolation=cv2.INTER_NEAREST)[None]
        for aa in AA[0, :, :, :, None].cpu().detach().numpy()
    ]
    img_grid = [
        img_grid[_].repeat(3, 0) *
        np.array(color(xx[_].item()))[:3, None, None] for _ in range(H * W)
    ]
