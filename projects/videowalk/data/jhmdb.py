...
class JhmdbSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        lbls_coord[0, :, :] = lbls_coord[0, :, :] * float(neww) / float(
            wd) / self.mapScale[0]
        lbls_coord[1, :, :] = lbls_coord[1, :, :] * float(newh) / float(
            ht) / self.mapScale[1]
        lblsize = (rsz_h, rsz_w)
        ...
        for i in range(lbls_coord.shape[2]):
            ...
            for j in range(lbls_coord.shape[1]):
                if self.sigma > 0:
                    draw_labelmap_np(lbls[i, :, :, j], lbls_coord_now[:, j],
                                     self.sigma)
                else:
                    tx = int(lbls_coord_now[0, j])
                    ty = int(lbls_coord_now[1, j])
                    if tx < lblsize[1] and ty < lblsize[
                            0] and tx >= 0 and ty >= 0:
                        lbls[i, ty, tx, j] = 1.0

        lbls_tensor = torch.zeros(frame_num, lblsize[0], lblsize[1],
                                  lbls_coord.shape[1])
        ...
        lbls_tensor = torch.cat([(lbls_tensor.sum(-1) == 0)[..., None] * 1.0,
                                 lbls_tensor],
                                dim=-1)

        lblset = np.arange(lbls_tensor.shape[-1] - 1)
        lblset = np.array([[0, 0, 0]] + [cm.Paired(i)[:3]
                                         for i in lblset]) * 255.0
