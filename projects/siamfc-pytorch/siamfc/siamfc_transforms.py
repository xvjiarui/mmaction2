...
class RandomCropBox(object):
    ...
    def __call__(self, box):
        box[:2] += np.clip(np.random.laplace(0, 1.0 / 4, 2), -1, 1) * (
            box[2:4] * self.size[0:2])
        return box
...
class SiamFCTransforms(object):
    ...
    def __init__(self,
        ...
        if self.make_label:
            self.y_grid, self.x_grid = np.ogrid[
    ...
    def _crop_and_stretch(self, img, box, box_transforms, make_label):
        # Faster version of their crop and stretch functions which only
        # computes the output image once instead of many times.
        ...
        xyxy = xywh_to_xyxy(box[:4])
        avg_color = np.mean(img, axis=(0, 1), dtype=float)
        img = get_cropped_input(img, xyxy, 1, box[4], cv2.INTER_LINEAR,
                                avg_color)[0]
        if make_label:
            center_diff = (box_start[:2] - box[:2]) / box[3] * self.label_size
            dist = np.abs(self.x_grid -
                          center_diff[0]) + np.abs(self.y_grid -
                                                   center_diff[1])
