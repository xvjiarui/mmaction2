...
parser.add_argument(
    '-o', '--out_folder', default='/scratch/ajabri/davis_results/', type=str)
parser.add_argument(
    '-i',
    '--in_folder',
    default='/scratch/ajabri/davis_results_masks/',
    type=str)
parser.add_argument('-d', '--dataset', default='/data/davis/DAVIS/', type=str)
...
def convert_dir(i):
    ...
    for j in range(len(files)):
        ...
        lblidx = cv2.resize(
            lblidx, (width, height), interpolation=cv2.INTER_NEAREST)
        lblidx = lblidx.astype(np.uint8)
