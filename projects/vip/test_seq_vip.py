...
def parse_args():
    ...
    parser.add_argument('--data-config', default='projects/vip/configs/vip_baseline.py', help='dataset config')
...
def main():
    ...
    log_file = osp.join(cfg.work_dir, f'test_seq_vip-{timestamp}.log')
    ...
    for h in cfg.log_config.hooks:
        if rank == 0 and h.type == 'WandbLoggerHook' and not args.disable_wandb:
            ...
            init_kwargs.update(
                dict(
                    name=h.init_kwargs.name + '-vip',
                    tags=[*h.init_kwargs.tags, 'vip'],
