...
def process_pose(pred, lbl_set, topk=3):
    ...
    current_coord = torch.stack([(xx * vals).sum(0), (yy * vals).sum(0)],
                                dim=0)
...
def dump_predictions(pred, lbl_set, img, prefix):
    ...
    pred_soft = cv2.resize(
        pred_soft, (img.shape[1], img.shape[0]),
...
def mem_efficient_batched_affinity(query, keys, mask, temperature, topk,
                                   long_mem, device):
...
def batched_affinity(query, keys, mask, temperature, topk, long_mem, device):
    """Mini-batched computation of affinity, for memory efficiency (less
    aggressively mini-batched)"""
    bsize = 2
    Ws, Is = [], []
    for b in range(0, keys.shape[2], bsize):
        _k, _q = keys[:, :,
...
def infer_downscale(model):
    out = model(
        torch.zeros(1, 10, 3, 320, 320).to(next(model.parameters()).device),
        just_feats=True)
    scale = out[1].shape[-2:]
