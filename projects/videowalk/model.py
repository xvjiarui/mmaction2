...
class CRW(nn.Module):
    ...
    def __init__(self, args, vis=None):
        ...
        self.temperature = getattr(args, 'temp',
                                   getattr(args, 'temperature', 0.07))
    ...
    def infer_dims(self):
        in_sz = 256
        dummy = torch.zeros(1, 3, 1, in_sz,
                            in_sz).to(next(self.encoder.parameters()).device)
    ...
    def zeroout_diag(self, A, zero=0):
        mask = (torch.eye(A.shape[-1]).unsqueeze(0).repeat(
            A.shape[0], 1, 1).bool() < 1).float().cuda()
        return A * mask
    ...
    def pixels_to_nodes(self, x):
        """pixel maps -> node embeddings Handles cases where input is a list of
        patches of images (N>1), or list of whole images (N=1)
    ...
    def forward(
        ...
        """Input is B x T x N*C x H x W, where either N>1 -> list of patches of
        images N=1 -> list of images."""
        B, T, C, H, W = x.shape
        _N, C = C // 3, 3
        ...
        if just_feats:
            h, w = np.ceil(np.array(x.shape[-2:]) /
                           self.map_scale).astype(np.int)
        ...
        # Visualizations
        #################################################################
        if (np.random.random() < 0.02) and (self.vis
    ...
    def visualize_frame_pair(self, x, q, mm):
        ...
        utils.visualize.frame_pair(x, q, mm, t1, t2, A, AA, xent_loss,
                                   self.vis.vis)
