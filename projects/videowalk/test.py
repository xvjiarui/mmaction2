...
def main(args, vis):
    ...
    dataset = (vos.VOSDataset
               if not 'jhmdb' in args.filelist else jhmdb.JhmdbSet)(
                   args)
...
def test(loader, model, args):
    ...
    for vid_idx, (imgs, imgs_orig, lbls, lbls_orig, lbl_map,
        ...
        with torch.no_grad():
            ...
            if args.pca_vis and vis:
                pca_feats = [
                    utils.visualize.pca_feats(feats[0].transpose(0, 1), K=1)
                ]
                for pf in pca_feats:
            ...
            Ws, Is = test_utils.mem_efficient_batched_affinity(
                query, keys, D, args.temperature, args.topk, args.long_mem,
                args.device)
            ...
            for t in range(key_indices.shape[0]):
                ...
                # Save Predictions
                cur_img = imgs_orig[0, t + n_context].permute(1, 2,
                                                              0).numpy() * 255
                _maps = []
                ...
                if 'VIP' in args.filelist:
                    outpath = os.path.join(
                        args.save_path, 'videos' +
                        meta['img_paths'][t +
                                          n_context][0].split('videos')[-1])
...
if __name__ == '__main__':
    ...
    if args.visdom:
        ...
        import wandb
        vis = visdom.Visdom(
            server=args.visdom_server, port=8095, env='main_davis_viz1')
        vis.close()
