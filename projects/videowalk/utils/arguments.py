...
def test_args():
    ...
    parser.add_argument(
        '--manualSeed', type=int, default=777, help='manual seed')
    ...
    parser.add_argument(
        '--temperature', default=0.07, type=float, help='temperature')
    ...
    parser.add_argument(
        '--filelist', default='/data/davis/DAVIS/val2017.txt', type=str)
...
def train_args():
    ...
    parser.add_argument(
        ...
        help=
        '/home/ajabri/data/places365_standard/train/ | /data/ajabri/kinetics/')
    ...
    parser.add_argument(
        '--output-dir', default='auto', help='path where to save')
    parser.add_argument('--resume', default='', help='resume from checkpoint')
    parser.add_argument(
        '--partial-reload',
        default='',
        help=
        'reload net from checkpoint, ignoring keys that are not in current model'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--dropout', default=0, type=float, help='dropout rate on A')
    parser.add_argument(
        '--zero-diagonal',
    ...
    parser.add_argument(
        '--frame-aug', default='', type=str, help='grid or none')
    parser.add_argument(
        '--frame-transforms',
        default='crop',
        type=str,
    ...
    parser.add_argument(
        '--server', default='localhost', type=str, help='visdom server')

    parser.add_argument(
        '--model-type',
        default='scratch',
        type=str,
        help='scratch | imagenet | moco')
    ...
    parser.add_argument(
        '--restrict', default=-1, type=int, help='restrict attention')
    parser.add_argument(
        '--head-depth',
        default=0,
        type=int,
    ...
    if args.output_dir == 'auto':
        ...
        name = '-'.join([
            '%s%s' %
            (keys[k],
             getattr(args, k) if not isinstance(getattr(args, k), list) else
             '-'.join([str(s) for s in getattr(args, k)])) for k in keys
        ])
        args.output_dir = 'checkpoints/%s_%s/' % (args.name, name)

        import datetime
        dt = datetime.datetime.today()
        args.name = '%s-%s-%s_%s' % (str(dt.month), str(
            dt.day), args.name, name)
