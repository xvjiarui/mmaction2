...
from . import arguments, augs, visualize
...
def load_vince_model(path):
    ...
    checkpoint = {
        k.replace('feature_extractor.module.model.', ''): checkpoint[k]
        for k in checkpoint if 'feature_extractor' in k
    }
    return checkpoint
...
def load_tc_model():
    ...
    for k in [k for k in model_state.keys() if 'encoderVideo' in k]:
        ...
        tmp = model_state[k]
        if net_state[kk].shape != model_state[k].shape and net_state[kk].dim(
        ) == 4 and model_state[k].dim() == 5:
...
class MaskedAttention(nn.Module):
    """A module that implements masked attention based on spatial locality TODO
    implement in a more efficient way (torch sparse or correlation filter)"""
