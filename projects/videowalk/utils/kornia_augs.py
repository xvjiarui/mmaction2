...
def patch_grid(x, transform, shape=(64, 64, 3), stride=[1.0, 1.0]):
    ...
    def transform(x):
        spatial_jitter = K.RandomResizedCrop(
            size=shape[:2], scale=(0.7, 0.9), ratio=(0.7, 1.3))

        import time
        t0 = time.time()
        x1 = x.unfold(2, 64, 32).unfold(3, 64, 32)
        t1 = time.time()
        x = kornia.contrib.extract_tensor_patches(
            x, window_size=shape[:2], stride=stride[:2])
        t2 = time.time()
        print(t2 - t1, t1 - t0)
...
def get_frame_aug(frame_aug, patch_size):
    ...
    if 'grid' in frame_aug:
        aug = patch_grid(
            x, transform=transform, shape=patch_size, stride=patch_size // 2)
    else:
        aug = transform
